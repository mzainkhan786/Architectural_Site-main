import Link from "next/link";
const ULinkButton = ({
  href = "/",
  text = "",
  className = "",
  color = "gold-gold",
}) => {
  const colorStyles = {
    // before element used to transition the gradient. not possible without it
    "gold-gold":
      // "max-w-[280px] w-full flex-center relative z-[1] bg-gradient-to-b from-accent-gold-lighter to-accent-gold-light shadow-btn before:bg-white before:opacity-0 before:z-[-1] before:absolute before:top-0.5 before:left-0.5 before:right-0.5 before:bottom-0.5 before:rounded-sm hover:text-accent-gold hover:from-accent-gold hover:to-accent-gold hover:before:opacity-100 hover:shadow-none before:transition-opacity before:duration-300 duration-300",
      "relative flex justify-center items-center w-[280px] lg:w-[240px] md:w-[165px] h-[45px] lg:h-[40px] md:h-[35px] !rounded-[10px] lg:!rounded-[8px] md:!rounded-[6px]  bg-gradient-to-b from-[#ffddb4] to-[#dcbb60] text-nowrap uppercase text-accent-black/80 hover:text-[#d9bf77] hover:bg-none hover:border-[3px] hover:border-[#d9bf77] transition-opacity duration-300",
    "gold-gray":
      "relative z-[1] bg-gradient-to-b from-accent-gold-lighter to-accent-gold-light shadow-btn before:bg-white before:opacity-0 before:z-[-1] before:absolute before:top-0.5 before:left-0.5 before:right-0.5 before:bottom-0.5 before:rounded-sm hover:text-black/70 hover:from-black/70 hover:to-black/70 hover:before:opacity-100 hover:shadow-none before:transition-opacity before:duration-300 duration-300",
    gray: "bg-accent-gray text-white",
    "solid-gold":
      "border-2 border-accent-gold-2 bg-accent-gold-2 text-black hover:bg-white transition-colors duration-300",
  };

  return (
    <>
      <Link
        href={href}
        className={`block  font-proxima-nova uppercase rounded ${colorStyles[color]} ${className}`}>
        {text}
      </Link>
    </>
  );
};

export default ULinkButton;
